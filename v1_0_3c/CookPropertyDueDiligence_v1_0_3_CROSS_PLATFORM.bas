...
' ============================================================================
' COOK COUNTY PROPERTY DUE DILIGENCE v1.0.3 - MAC + WINDOWS EXCEL
'
' Design:
...
Private Sub SafeAdapter(ByVal sourceName As String, ByVal procName As String, ByVal pin As String)
    ...
    If StrComp(sourceName, "Enterprise Zone", vbTextCompare) = 0 Then
        UpdateSourceStatus sourceName, "PARTIAL", "Finished - boundary verification remains manual", _
                           "Automatic boundary result intentionally disabled; verify the parcel in the official Illinois DCEO map."
    Else
...
Private Sub FetchEnterpriseZone(ByVal pin As String)
    ...
    SetProperty "Enterprise Zone (DCEO)", "Manual boundary verification required", _
                "Illinois DCEO", "Current official map", "PARTIAL", _
                "The official DCEO map is authoritative for this check; no unverified statewide FeatureServer is used."
    ...
    AppendIncentive "Illinois Enterprise Zone - DCEO Verification", _
                    ...
                    "Authoritative statewide FeatureServer has not been positively resolved and regression-tested; do not guess.", appUrl

    AppendIssue "MEDIUM", "Enterprise Zone boundary status was not automatically resolved.", _
                "Illinois DCEO Enterprise Zone Map", _
                "Verify the parcel in the official DCEO interactive map before relying on Enterprise Zone eligibility."
End Sub
...
Private Sub BuildReport()
    ...
    ws.Range("E11").Value = FindIncentiveResult("Illinois Enterprise Zone - DCEO Verification")
