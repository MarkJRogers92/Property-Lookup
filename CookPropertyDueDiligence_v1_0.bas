...
' ============================================================================
' COOK COUNTY PROPERTY DUE DILIGENCE v1.0 - STANDALONE WINDOWS EXCEL
'
' Design:
...
'
' IMPORTANT:
'   Enterprise Zone uses the Assessor Parcel Universe signal when available,
'   but the official DCEO map remains the boundary-verification source until
'   its authoritative statewide FeatureServer is positively resolved/tested.
' ============================================================================
...
Private Sub SafeAdapter(ByVal sourceName As String, ByVal procName As String, ByVal pin As String)
    ...
    If StrComp(sourceName, "Enterprise Zone", vbTextCompare) = 0 Then
        UpdateSourceStatus sourceName, "PARTIAL", "Finished - boundary verification remains manual", _
                           "Assessor signal collected when available; verify parcel boundary in the official Illinois DCEO map."
    Else
...
Private Sub FetchUniverse(ByVal pin As String)
    ...
    If d.Exists("econ_enterprise_zone_num") Then
        ezNum = DictGet(d, "econ_enterprise_zone_num")
        SetProperty "Enterprise Zone # (Assessor signal)", ezNum, "Parcel Universe", Nz(DictGet(d, "year")), "CHECK", _
                    "Assessor signal only. Verify actual boundary in the official Illinois DCEO Enterprise Zone map."
    Else
        AppendIssue "MEDIUM", "Parcel Universe did not expose the expected Enterprise Zone signal field.", _
                    "Assessor Parcel Universe", _
                    "Use the official Illinois DCEO Enterprise Zone map for boundary verification; do not infer a No result from the missing field."
    End If
End Sub
...
Private Sub FetchBORAppeals(ByVal pin As String)
    ...
    For i = 2 To rows.Count
        ...
        AppendAppeal "Board of Review", DictGet(d,"tax_year"), _
                     ...
                     FirstNonBlank(d, Array("appealtypedescription","appealtype")), "", _
...
Private Sub FetchTaxPortal(ByVal pin As String)
    ...
    Dim re As Object, ms As Object, m As Object, r As Long, yr As String
    Dim statusText As String, starText As String, exText As String, firstBill As Boolean
    Dim portalBase As String

    portalBase = CStr(GetConfigValue("Property Tax Portal Results URL", _
        "https://www.cookcountypropertyinfo.com/PINResults.aspx"))
    url = portalBase & "?pin=" & pin
    html = HttpGet(url)
    txt = CollapseWhitespace(HtmlToText(html))

    If InStr(1, txt, FormatPin(pin), vbTextCompare) = 0 Or _
       InStr(1, txt, "PROPERTY CHARACTERISTICS", vbTextCompare) = 0 Then
        Err.Raise vbObjectError + 160, , "Property Tax Portal did not return a PIN-specific results page for " & FormatPin(pin) & "."
    End If

    rate = RegexFirst(txt, "Tax Rate\s*:\s*([0-9\.]+)", 1)
    taxCode = RegexFirst(txt, "Tax Code\s*:\s*([0-9]+)", 1)
    portalClass = RegexFirst(txt, "Property Class\s*:\s*([0-9\-]+)", 1)
    portalLot = RegexFirst(txt, "Lot Size \(SqFt\)\s*:\s*([0-9,\.]+)", 1)
    portalBldg = RegexFirst(txt, "Building \(SqFt\)\s*:\s*([0-9,\.]+)", 1)
    ...
    If Len(portalClass) > 0 And Len(GetPropertyValue("Assessor Class")) > 0 Then
        If NormalizeCompare(portalClass) <> NormalizeCompare(GetPropertyValue("Assessor Class")) Then
            AppendIssue "MEDIUM", "Property Tax Portal class differs from Parcel Universe class.", _
                        "Property Tax Portal / Parcel Universe", _
                        "Portal: " & portalClass & "; Parcel Universe: " & GetPropertyValue("Assessor Class") & ". Verify current classification."
        End If
    End If
    If Len(portalClassDesc) > 0 And Len(GetPropertyValue("GIS Class Description")) = 0 Then
        SetProperty "GIS Class Description", portalClassDesc, "Property Tax Portal", "Live", "CHECK", "Portal class description used because GIS description was blank."
    End If
    ...
    ' Composite tax-rate history.
    rateBlock = TextBetween(txt, "Tax Rate History", "Tax Code")
    re.Pattern = "(20[0-9]{2})\s+([0-9\.]+)"
    Set ms = re.Execute(rateBlock)
    For Each m In ms
        UpsertTaxHistory CStr(m.SubMatches(0)), "", "", CStr(m.SubMatches(1)), taxCode, ""
    Next m

    ' Five-year billed amounts and current-year payment/balance status.
    ...
    re.Pattern = "(20[0-9]{2})\s*:\s*\$?([0-9,\.]+)(\*)?\s*(Paid in Full|Pay Online:\s*\$?[0-9,\.]+|Payment History)?"
    Set ms = re.Execute(billBlock)
    firstBill = True
    For Each m In ms
        statusText = Trim$(CStr(m.SubMatches(3)))
        starText = Trim$(CStr(m.SubMatches(2)))
        If Len(statusText) = 0 And Len(starText) > 0 Then statusText = "Partial-year / installment amount shown"
        UpsertTaxHistory CStr(m.SubMatches(0)), CStr(m.SubMatches(1)), statusText, "", taxCode, ""
        If firstBill Then
            If Len(starText) > 0 Then
                ThisWorkbook.Worksheets("Report").Range("E9").Value = "$" & CStr(m.SubMatches(1)) & " (1st installment)"
            Else
                ThisWorkbook.Worksheets("Report").Range("E9").Value = "$" & CStr(m.SubMatches(1))
            End If
            firstBill = False
        End If
    Next m

    ' Five-year exemption history.
    ...
    If Len(refundBlock) > 0 And InStr(1, refundBlock, "No Refund Available", vbTextCompare) = 0 Then
...
Private Sub FetchEnterpriseZone(ByVal pin As String)
    Dim appUrl As String, ezNum As String
    ...
    ezNum = Trim$(GetPropertyValue("Enterprise Zone # (Assessor signal)"))

    If Len(ezNum) > 0 And ezNum <> "0" Then
        AppendIncentive "Illinois Enterprise Zone - Assessor Signal", "Signal present", ezNum, _
                        "Assessor Parcel Universe", _
                        "This is an Assessor data signal, not a substitute for boundary verification.", _
                        "https://datacatalog.cookcountyil.gov/d/pabr-t5kh"
        AppendIssue "MEDIUM", "Assessor data indicates an Enterprise Zone signal.", _
                    "Parcel Universe / Illinois DCEO", _
                    "Verify the parcel boundary in the official DCEO Enterprise Zone map before relying on eligibility."
    Else
        AppendIncentive "Illinois Enterprise Zone - Assessor Signal", "No signal returned", "", _
                        "Assessor Parcel Universe", _
                        "Absence of the Assessor signal is not treated as a conclusive boundary determination.", _
                        "https://datacatalog.cookcountyil.gov/d/pabr-t5kh"
    End If

    AppendIncentive "Illinois Enterprise Zone - DCEO Verification", "Manual boundary verification", "", _
                    "Official Illinois DCEO Enterprise Zone map", _
                    "Authoritative statewide FeatureServer has not been positively resolved/tested; do not guess.", appUrl
End Sub
...
' Excel header/footer text uses "&" to introduce formatting codes (e.g. "&B" = bold), so a literal
' "&" in printed text (a corner-lot address like "MAIN ST & 5TH AVE") must be doubled or it is
' swallowed/misinterpreted.
...
' Counts ArcGIS REST "attributes" objects in a query response (one per returned feature). This is
' used instead of pattern-matching the literal text "features":[] to decide "no results", because
' that literal match depends on getting Esri's exact whitespace convention right (f=pjson vs f=json,
' and conventions can differ by server/version) - if the assumption is wrong, "no intersection"
' silently reads as "intersection found" instead, which is the wrong failure direction for a
' due-diligence tool. Counting "attributes" occurrences with a whitespace-tolerant regex works
...
Private Function SocrataCsv(ByVal datasetId As String, ByVal pin As String, ByVal extraQuery As String) As String
    ...
    ' extraQuery carries plain SoQL like "$order=year DESC", which contains a literal space.
    ' A raw space inside a URL is not valid in an HTTP request line, so encode it after assembly
    ' rather than requiring every caller to pre-encode its SoQL.
...
Private Sub ParsePortalTaxSaleStatuses(ByVal blockText As String)
    Dim re As Object, ms As Object, m As Object, seen As Object
    Dim yr As String, seg As String, statusText As String
    ...
    re.Global = True
    re.IgnoreCase = True
    re.Pattern = "(20[0-9]{2})\s*:"
    Set ms = re.Execute(blockText)

    For Each m In ms
        yr = CStr(m.SubMatches(0))
        If Not seen.Exists(yr) Then
            seen.Add yr, True
            seg = PortalYearSegment(blockText, yr)

            If InStr(1, seg, "Tax Sale Has Not Occurred", vbTextCompare) > 0 Or _
               InStr(1, seg, "Tax Sale Has Not Occureed", vbTextCompare) > 0 Then
            ...
            Else
                ...
                If Len(statusText) > 0 Then
                    AppendIssue "HIGH", "Tax-sale/delinquency section returned a nonstandard status for " & yr & ".", _
                                "Cook County Property Tax Portal", _
                                "Review the Portal and Clerk/Treasurer records before relying on tax status. Portal text: " & statusText
...
Private Sub ParsePortalDocuments(ByVal txt As String)
    Dim blockText As String, re As Object, ms As Object, m As Object, seen As Object
    Dim docNo As String, docType As String, recDate As String
    ...
    re.Global = True
    re.IgnoreCase = True
    re.Pattern = "([0-9]{8,12})\s*-\s*([A-Z0-9][A-Z0-9 &'\.\-]{0,59})\s*-\s*([0-9]{2}/[0-9]{2}/[0-9]{4})"
    Set ms = re.Execute(blockText)

    For Each m In ms
        docNo = Trim$(CStr(m.SubMatches(0)))
        docType = Trim$(CStr(m.SubMatches(1)))
        recDate = Trim$(CStr(m.SubMatches(2)))
        If Len(docNo) > 0 And Not seen.Exists(docNo) Then
            seen.Add docNo, True
            AppendDocument docNo, docType, recDate

            If InStr(1, docType, "LIS PENDENS", vbTextCompare) > 0 Or _
               InStr(1, docType, "FORECLOSURE", vbTextCompare) > 0 Then
                AppendIssue "HIGH", "Recent recorded-document index includes " & docType & ".", _
                            "Cook County Property Tax Portal Documents / Deeds / Liens", _
                            "Obtain and review document " & docNo & " recorded " & recDate & " from the Cook County Clerk/Recorder."
            ElseIf InStr(1, docType, "LIEN", vbTextCompare) > 0 Then
                AppendIssue "MEDIUM", "Recent recorded-document index includes " & docType & ".", _
                            "Cook County Property Tax Portal Documents / Deeds / Liens", _
                            "Obtain and review document " & docNo & " recorded " & recDate & "."
            End If
        End If
    Next m
End Sub
...
Private Function PortalExemptionForYear(ByVal blockText As String, ByVal taxYear As String) As String
    ...
    countText = RegexFirst(seg, "([0-9]+)\s+Exemptions Received", 1)
